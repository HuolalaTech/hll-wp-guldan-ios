//
//  GDNRecordDetailViewController.m
//  Guldan
//
...
static CGFloat GDNScrollWidth = 600;
static CGFloat GDNHeaderHight = 140;

@interface GDNRecordDetailViewController ()<UITableViewDataSource, GDNRecordDetailCellDelegate>
...
@implementation GDNRecordDetailViewController

- (instancetype)initWithModel:(GDNUIModel *)model {
    self = [super init];
    if (self) {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (self.tpTableType == tableTypeSequential) {
        GDNRecordHierarchyModel *model = self.sequentialMethodRecord[section];
    ...
    {
        GDNRecordHierarchyModel *model = self.costTimeSortMethodRecord[section];
        if (model.isExpand && [model.subMethods isKindOfClass:NSArray.class]) {
            return model.subMethods.count+1;
        }
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *GDNRecordCellId = @"GDNRecordDetailCell_reuseIdentifier";
    GDNRecordDetailCell *cell = [tableView dequeueReusableCellWithIdentifier:GDNRecordCellId];
    if (!cell) {
        cell = [[GDNRecordDetailCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:GDNRecordCellId];
    }
    GDNRecordHierarchyModel *model;
    GDNRecordModel *recordModel;
...
- (UIScrollView *)tpScrollView {
    if (!_tpScrollView) {
        _tpScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, GDNHeaderHight, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - GDNHeaderHight)];
        _tpScrollView.showsHorizontalScrollIndicator = YES;
        _tpScrollView.alwaysBounceHorizontal = YES;
        _tpScrollView.contentSize = CGSizeMake(GDNScrollWidth, 0);
    }
...
- (UITableView *)tpTableView {
    if (!_tpTableView) {
        _tpTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 30, GDNScrollWidth, [UIScreen mainScreen].bounds.size.height - GDNHeaderHight - 30) style:UITableViewStylePlain];
...
- (UILabel *)tableHeaderViewLabel {
    if (!_tableHeaderViewLabel) {
        _tableHeaderViewLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, GDNScrollWidth, 30)];
